...
public class SubscribeOrderEventDeserializer extends JsonDeserializer<SubscribeOrderEvent> {
  ...
  @Override
  public SubscribeOrderEvent deserialize(JsonParser jp, DeserializationContext ctx) throws IOException {
    ...
    if(json.contains("\"table\":\"order\"")){
      try{
        SubscribeOrderEvent event = new SubscribeOrderEvent();
        List<SubscribeOrderData> data = mapper.readerFor(new TypeReference<List<SubscribeOrderData>>(){}).readValue(node.get("data"));
